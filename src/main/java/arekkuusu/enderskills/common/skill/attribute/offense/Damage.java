            ...
            if (c.isOwned(this)) {
                if (!GuiScreen.isShiftKeyDown()) {
                    ...
                    c.getOwned(this).ifPresent(skillInfo -> {
                        ...
                        if (Configuration.getSyncValues().extra.multiplyDamage) {
                            description.add(TextHelper.translate("desc.stats.dmg", TextHelper.format2FloatPoint(getModifierMultiplication(attributeInfo) * 100), TextHelper.getTextComponent("desc.stats.suffix_percentage")));
                        } else {
                            description.add(TextHelper.translate("desc.stats.dmg", TextHelper.format2FloatPoint(getModifierAddition(attributeInfo) / 20D), TextHelper.getTextComponent("desc.stats.suffix_hearts")));
                        }
                        if (attributeInfo.getLevel() < getMaxLevel()) { //Copy info and set a higher level...
                            ...
                            if (Configuration.getSyncValues().extra.multiplyDamage) {
                                description.add(TextHelper.translate("desc.stats.dmg", TextHelper.format2FloatPoint(getModifierMultiplication(infoNew) * 100), TextHelper.getTextComponent("desc.stats.suffix_percentage")));
                            } else {
                                description.add(TextHelper.translate("desc.stats.dmg", TextHelper.format2FloatPoint(getModifierAddition(infoNew) / 20D), TextHelper.getTextComponent("desc.stats.suffix_hearts")));
                            }
                        }
                    });
                }
