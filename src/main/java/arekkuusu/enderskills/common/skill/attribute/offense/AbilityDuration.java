...
public class AbilityDuration extends BaseAttribute implements ISkillAdvancement {
        ...
        Capabilities.get(entity).ifPresent(capability -> {
            if (capability.isOwned(this)) {
                capability.getOwned(this).ifPresent(skillInfo -> {
                    ...
                    ABILITY_DURATION_ATTRIBUTE.apply(entity, getModifier(attributeInfo));
                });
            } else {
            ...
            if (c.isOwned(this)) {
                if (!GuiScreen.isShiftKeyDown()) {
                    ...
                    c.getOwned(this).ifPresent(skillInfo -> {
                        ...
                        if (Configuration.CONFIG_SYNC.applyAs == Configuration.Damage.MULTIPLICATION) {
                            description.add(TextHelper.translate("desc.stats.ad", TextHelper.format2FloatPoint(getModifier(attributeInfo) * 100), TextHelper.getTextComponent("desc.stats.suffix_percentage")));
                        } else {
                            description.add(TextHelper.translate("desc.stats.ad", TextHelper.format2FloatPoint(getModifier(attributeInfo))));
                        }
                        if (attributeInfo.getLevel() < getMaxLevel()) {
                            ...
                            if (Configuration.CONFIG_SYNC.applyAs == Configuration.Damage.MULTIPLICATION) {
                                description.add(TextHelper.translate("desc.stats.ad", TextHelper.format2FloatPoint(getModifier(infoNew) * 100), TextHelper.getTextComponent("desc.stats.suffix_percentage")));
                            } else {
                                description.add(TextHelper.translate("desc.stats.ad", TextHelper.format2FloatPoint(getModifier(infoNew))));
                            }
                            }
                        }
                    });
