...
public class HealOther extends BaseAbility implements IImpact, ISkillAdvancement {

    public HealOther() {
        super(LibNames.HEAL_OTHER);
        setTexture(ResourceLibrary.HEAL_OTHER);
    ...
    @Config(modid = LibMod.MOD_ID, name = LibMod.MOD_ID + "/Ability/" + LibNames.HEAL_OTHER)
    public static class Configuration {

        @Config.Comment("Ability Values")
        @Config.LangKey(LibMod.MOD_ID + ".config." + LibNames.HEAL_OTHER)
        ...
        public static class Values {
            ...
            public String[] cooldown = {
                    "(0+){48 * 20 + 12 * 20 * (1 - ((1 - (e^(-2.1 * (x/49)))) / (1 - e^(-2.1))))}",
                    "(50+){42 * 20 + 6 * 20 * (1- (((e^(0.1 * ((x-49) / (y-49))) - 1)/((e^0.1) - 1))))}",
                    "(100){(40 * 20)}"
            };
