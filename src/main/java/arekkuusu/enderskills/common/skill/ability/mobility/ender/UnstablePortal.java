...
public class UnstablePortal extends BaseAbility implements IImpact, IExpand, IScanEntities, ILoopSound, ISkillAdvancement {
    ...
    public void use(EntityLivingBase owner, SkillInfo skillInfo) {
        ...
        if (isActionable(owner) && canActivate(owner)) {
            ...
            double range = arekkuusu.enderskills.api.event.SkillRangeEvent.getRange(owner, this, getPortalRange(abilityInfo));;
    ...
    public float getPortalTeleport(AbilityInfo info) {
        return (float) this.config.get(this, "DISPLACEMENT", info.getLevel());
    }
            ...
            if (c.isOwned(this)) {
                if (!GuiScreen.isShiftKeyDown()) {
                    ...
                    c.getOwned(this).ifPresent(skillInfo -> {
                        ...
                        description.add(TextHelper.translate("desc.stats.portal_range", TextHelper.format2FloatPoint(getPortalRange(abilityInfo)), TextHelper.getTextComponent("desc.stats.suffix_blocks")));
                        description.add(TextHelper.translate("desc.stats.portal_teleport_range", TextHelper.format2FloatPoint(getPortalTeleport(abilityInfo)), TextHelper.getTextComponent("desc.stats.suffix_blocks")));
                        if (abilityInfo.getLevel() < getMaxLevel()) {
                            if (!GuiScreen.isCtrlKeyDown()) {
                                ...
                                description.add(TextHelper.translate("desc.stats.portal_range", TextHelper.format2FloatPoint(getPortalRange(infoNew)), TextHelper.getTextComponent("desc.stats.suffix_blocks")));
                                description.add(TextHelper.translate("desc.stats.portal_teleport_range", TextHelper.format2FloatPoint(getPortalTeleport(infoNew)), TextHelper.getTextComponent("desc.stats.suffix_blocks")));
                            }
                        }
                    });
                }
