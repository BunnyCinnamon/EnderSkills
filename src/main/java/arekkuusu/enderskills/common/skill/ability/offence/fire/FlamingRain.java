...
public class FlamingRain extends BaseAbility implements IImpact, ILoopSound, IExpand, IScanEntities, ISkillAdvancement {
    ...
    public void use(EntityLivingBase owner, SkillInfo skillInfo) {
        ...
        if (isActionable(owner) && canActivate(owner)) {
            ...
            double range = getRainRange(abilityInfo);
    ...
    public double getRainRange(AbilityInfo info) {
        ...
        double func = ExpressionHelper.getExpression(this, Configuration.getSyncValues().extra.rainRange, level, levelMax);
        double result = (func * CommonConfig.getSyncValues().skill.globalRange);
        return (result * getEffectiveness());
    }

    public int getRainDuration(AbilityInfo info) {
        int level = getLevel(info);
        int levelMax = getMaxLevel();
        double func = ExpressionHelper.getExpression(this, Configuration.getSyncValues().extra.rainDuration, level, levelMax);
            ...
            if (c.isOwned(this)) {
                if (!GuiScreen.isShiftKeyDown()) {
                    ...
                    c.getOwned(this).ifPresent(skillInfo -> {
                        ...
                        description.add(TextHelper.translate("desc.stats.rain_duration", TextHelper.format2FloatPoint(getRainDuration(abilityInfo) / 20D), TextHelper.getTextComponent("desc.stats.suffix_time")));
                        description.add(TextHelper.translate("desc.stats.rain_range", TextHelper.format2FloatPoint(getRainRange(abilityInfo)), TextHelper.getTextComponent("desc.stats.suffix_blocks")));
                        ...
                        if (abilityInfo.getLevel() < getMaxLevel()) { //Copy info and set a higher level...
                            ...
                            description.add(TextHelper.translate("desc.stats.rain_duration", TextHelper.format2FloatPoint(getRainDuration(infoNew) / 20D), TextHelper.getTextComponent("desc.stats.suffix_time")));
                            description.add(TextHelper.translate("desc.stats.rain_range", TextHelper.format2FloatPoint(getRainRange(infoNew)), TextHelper.getTextComponent("desc.stats.suffix_blocks")));
    ...
    public void initSyncConfig() {
        ...
        Configuration.getSyncValues().extra.rainDuration = Configuration.getValues().extra.rainDuration;
        Configuration.getSyncValues().extra.rainRange = Configuration.getValues().extra.rainRange;
    ...
    @Config(modid = LibMod.MOD_ID, name = LibMod.MOD_ID + "/Ability/" + LibNames.FLAMING_RAIN)
    public static class Configuration {

        @Config.Comment("Ability Values")
        @Config.LangKey(LibMod.MOD_ID + ".config." + LibNames.FLAMING_RAIN)
        ...
        public static class Values {
            ...
            public String[] cooldown = {
                    "(0+){19 * 20 + 2 * 20 * (1 - ((1 - (e^(-2.1 * (x/24)))) / (1 - e^(-2.1))))}",
                    "(25+){18.5 * 20 + 0.5 * 20 * (1- (((e^(0.1 * ((x-24) / (y-24))) - 1)/((e^0.1) - 1))))}",
                    "(50){18 * 20}"
            };
            ...
            public String[] range = {
                    "(0+){15 + 15 * (1 - (e^(-2.1 * (x/24)))) / (1 - e^(-2.1))}",
                    "(25+){30 + 5 * ((e^(0.1 * ((x - 24) / (y - 24))) - 1)/((e^0.1) - 1))}"
            };
            ...
            public static class Extra {
                ...
                public String[] dot = {
                        ...
                        "(25+){25.77 + ((e^(2.25 * ((x-24) / (y-24))) - 1)/((e^2.25) - 1)) * (45 - 25.77)}",
                        "(50){48}"
                };
