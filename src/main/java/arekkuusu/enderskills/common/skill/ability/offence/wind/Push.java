...
public class Push extends BaseAbility implements IImpact, IScanEntities, IExpand, IFindEntity, ISkillAdvancement {
    ...
    public static class Configuration {
        ...
        public static class Values {
            ...
            public String[] cooldown = {"(0+){(8 * 20) + (6 * 20) * (1 - ((e^(-0.1 * (x / y)) - 1)/((e^-0.1) - 1)))}"};

            @Config.Comment("Duration Function f(x,y)=? where 'x' is [Current Level] and 'y' is [Max Level]")
            public String[] time = {"(0+){3 * 20 + ((e^(-0.1 * (x / y)) - 1)/((e^-0.1) - 1)) * (8 * 20 - 3 * 20)}"};
            ...
            public static class Extra {
                ...
                public String[] pushRange = {"(0+){4 + ((e^(-0.1 * (x / y)) - 1)/((e^-0.1) - 1)) * (8 - 4)}"};
                @Config.Comment("Push Force Function f(x,y)=? where 'x' is [Current Level] and 'y' is [Max Level]")
                public String[] push = {"(0+){6 + ((e^(0.1 * (x / y)) - 1)/((e^0.1) - 1)) * (40 - 6)}"};
            }
