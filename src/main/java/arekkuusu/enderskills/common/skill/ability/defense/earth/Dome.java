...
public class Dome extends BaseAbility implements ISkillAdvancement {
    ...
    public Dome() {
        super(LibNames.DOME);
        setTexture(ResourceLibrary.DOME);
    }
    ...
    public void use(EntityLivingBase user, SkillInfo skillInfo) {
        ...
        if (state.isOpaqueCube() && state.isFullBlock() && shouldUse(user) && canUse(user)) {
            ...
            //EnderSkills.getProxy().playSound(user.world, user.getPositionVector(), ModSounds.DOME_UP, SoundCategory.PLAYERS, 5.0F);
    ...
    public static class Configuration {
        ...
        public static class Values {
            ...
            public String cooldown = "(18 * 20) + (38 * 20) * (1 - ((e^(-0.1 * (x / y)) - 1)/((e^-0.1) - 1)))";

            @Config.Comment("Duration Function f(x,y)=? where 'x' is [Current Level] and 'y' is [Max Level]")
            public String time = "4 * 20 + ((e^(-0.1 * (x / y)) - 1)/((e^-0.1) - 1)) * (12 * 20 - 4 * 20)";
