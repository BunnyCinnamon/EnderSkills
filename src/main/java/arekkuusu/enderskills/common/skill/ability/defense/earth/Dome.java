...
public class Dome extends BaseAbility implements ISkillAdvancement {

    public Dome() {
        super(LibNames.DOME, new AbilityProperties());
    ...
    public void use(EntityLivingBase owner, SkillInfo skillInfo) {
        ...
        if (state.isOpaqueCube() && state.isFullBlock() && isActionable(owner) && canActivate(owner)) {
            ...
            int width = 1 + size * 2;
            wall.create(pos.add(size, 0, 0), EnumFacing.EAST, width, size, time);
            wall.create(pos.add(-size, 0, 0), EnumFacing.WEST, width, size, time);
            ...
            if (c.isOwned(this)) {
                if (!GuiScreen.isShiftKeyDown()) {
                    ...
                    c.getOwned(this).ifPresent(skillInfo -> {
                        ...
                        description.add(TextHelper.translate("desc.stats.launch_force", TextHelper.format2FloatPoint(getLaunch(abilityInfo)) + "  m/s"));
                        if (abilityInfo.getLevel() < getMaxLevel()) { //Copy info and set a higher level...
                            ...
                            description.add(TextHelper.translate("desc.stats.launch_force", TextHelper.format2FloatPoint(getLaunch(infoNew)) + "  m/s"));
                        }
                    });
                }
            }
    ...
    @Config(modid = LibMod.MOD_ID, name = LibMod.MOD_ID + "/Ability/" + LibNames.DOME)
    public static class Configuration {

        @Config.Comment("Ability Values")
        @Config.LangKey(LibMod.MOD_ID + ".config." + LibNames.DOME)
        ...
        public static class Values {
            ...
            public String[] cooldown = {
                    "(0+){32 * 20 + 24 * 20 * (1 - ((1 - (e^(-2.1 * (x/24)))) / (1 - e^(-2.1))))}",
            ...
            public String[] time = {
                    "(0+){4 * 20 + 5 * 20 * (1 - (e^(-2.1 * (x/24)))) / (1 - e^(-2.1))}",
