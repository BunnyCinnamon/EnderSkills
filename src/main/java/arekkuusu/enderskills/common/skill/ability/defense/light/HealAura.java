...
public class HealAura extends BaseAbility implements IScanEntities, IExpand, IFindEntity, ISkillAdvancement {
    ...
    public HealAura() {
        super(LibNames.HEAL_AURA);
        setTexture(ResourceLibrary.HEAL_AURA);
    }
    ...
    public void begin(EntityLivingBase target, SkillData data) {
        ...
        Optional.ofNullable(NBTHelper.getEntity(EntityLivingBase.class, data.nbt, "user")).ifPresent(user -> {
            Capabilities.get(user).flatMap(s -> s.get(this)).ifPresent(skillInfo -> {
                AbilityInfo abilityInfo = (AbilityInfo) skillInfo;
                double healing = getHeal(abilityInfo);
                target.heal((float) (target.getMaxHealth() * healing));
            });
        });
    }
    ...
    public static class Configuration {
        ...
        @Config.LangKey(LibMod.MOD_ID + ".config." + LibNames.HEAL_AURA)
        ...
        public static class Values {
            ...
            public String[] range = {
                    "(0+){4 + 7 * (1 - (e^(-2.1 * (x/49)))) / (1 - e^(-2.1))}",
                    "(50+){11 + 3 * ((e^(0.1 * ((x - 49) / (y - 49))) - 1)/((e^0.1) - 1))}"
            };
