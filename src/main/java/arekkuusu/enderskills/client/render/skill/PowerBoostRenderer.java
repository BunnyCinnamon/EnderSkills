...
public class PowerBoostRenderer extends SkillRenderer<PowerBoost> {
    ...
    public PowerBoostRenderer() {
        ...
        EntityThrowableDataRenderer.add(ModAbilities.POWER_BOOST, ProjectileLightRenderer::new);
    }
    ...
    public static class Events {
        ...
        public void onLivingPostRender(RenderLivingEvent.Post<EntityLivingBase> event) {
            ...
            if (getTick(event.getEntity()) > 0) {
                ...
                GlStateManager.color(0.38F, 0.19F, 0.608F, 1F * (getTick(entity) / 10F));
        ...
        public void onRenderLivingPre(RenderLivingEvent.Pre<EntityLivingBase> event) {
            if (getTick(event.getEntity()) > 0 && !rendering) {
                ...
                int tick = getTick(event.getEntity());
                float scale = 1F + (size * (tick / 10F));

                RenderLivingBase<EntityLivingBase> render = event.getRenderer();
                GlStateManager.pushMatrix();
                GlStateManager.translate(event.getX(), event.getY(), event.getZ());
                GlStateManager.scale(scale, scale, scale);
                render.doRender(event.getEntity(), 0F, 0F, 0F, yaw, event.getPartialRenderTick());
                GlStateManager.popMatrix();
                rendering = false;
            }
        }
        ...
        public void onLivingUpdate(LivingEvent.LivingUpdateEvent event) {
            ...
            boolean active = SkillHelper.isActive(event.getEntity(), ModAbilities.POWER_BOOST);
            //Handle tick
            if (active) {
        ...
        public final String tickKey = ModAbilities.POWER_BOOST.getRegistryName() + ":tick";
