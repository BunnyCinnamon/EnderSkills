...
public class PushRenderer extends SkillRenderer<Push> {
    ...
    public static class Events {
        ...
        public void onRenderPre(RenderLivingEvent.Pre<EntityLivingBase> event) {
            SkillHelper.getActiveNotOwner(event.getEntity(), ModAbilities.PUSH, holder -> {
                if (holder.tick > 10) {
            ...
            if (entity.tick % 2 == 0) {
                ...
                for (int i = 0; i < 2; ++i) {
                    for (int j = 1; j < 2; ++j) {
                        ...
                        ParticleVanilla vanilla = new ParticleVanilla(entity.world, new Vec3d(posX, posY, posZ), new Vec3d(speedVec.x, speedVec.y, speedVec.z), 5F, 25, 0xFFFFFF, 0);
                        Minecraft.getMinecraft().effectRenderer.addEffect(vanilla);
                    }
                }
            }
