...
public class DashRenderer extends SkillRenderer<Dash> {
    ...
    public void render(Entity entity, double x, double y, double z, float partialTicks, SkillHolder skillHolder) {
        if (skillHolder.tick == 1) {
            for (int i = 0; i < 12; i++) {
                if (entity.world.rand.nextDouble() < 0.4D && ClientProxy.canParticleSpawn()) {
                    ...
                    ParticleVanilla vanilla = new ParticleVanilla(entity.world, new Vec3d(posX, posY, posZ), new Vec3d(0, 0, 0), 4F, 18, 0xFFFFFF, 0);
