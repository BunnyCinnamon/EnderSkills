...
public class ProjectileWispRenderer extends Render<EntityThrowableData> {

    protected ProjectileWispRenderer(RenderManager renderManager) {
        super(renderManager);
    }
    ...
    public void doRender(EntityThrowableData entity, double x, double y, double z, float entityYaw, float partialTicks) {
        ...
        for (int i = 0; i < 3; i++) {
            if (ClientProxy.canParticleSpawn()) {
                ...
                EnderSkills.getProxy().spawnParticle(entity.world, posVec.toVec3d(), speedVec.toVec3d(), particleScale, 10, 0x7AFFFC, ResourceLibrary.GLOW);
            }
        }
