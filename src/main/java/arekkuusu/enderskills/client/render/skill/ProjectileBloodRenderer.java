...
public class ProjectileBloodRenderer extends Render<EntityThrowableData> {

    protected ProjectileBloodRenderer(RenderManager renderManager) {
        super(renderManager);
    }
    ...
    public void doRender(EntityThrowableData entity, double x, double y, double z, float entityYaw, float partialTicks) {
        if (entity.world.rand.nextDouble() < 0.3D && ClientProxy.canParticleSpawn()) {
            for (int i = 1; i <= 3; i++) {
                ...
                EnderSkills.getProxy().spawnParticle(entity.world, posVec.toVec3d(), new Vec3d(0, 0, 0), 3F, 25, 0x690303, ResourceLibrary.MOTE);
            }
        }
    }
